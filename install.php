    ...
    if ($action === 'run') {
        try {
            ...
            $tables = [
                ...
                // 23. EMAIL TRACKING (MASTER RECIPIENT PIXEL RECORDS)
                "CREATE TABLE IF NOT EXISTS `email_tracking` (
                    ...
                    `apple_privacy_count` INT NOT NULL DEFAULT 0,
                    `gmail_proxy_count`   INT NOT NULL DEFAULT 0,
                    `created_at`          TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
                    ...
                    INDEX `idx_et_sent` (`sent_at`)
                ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4",

                // 24. EMAIL OPEN EVENTS (INDIVIDUAL OPENS LOG)
                "CREATE TABLE IF NOT EXISTS `email_open_events` (
                    `id`               BIGINT AUTO_INCREMENT PRIMARY KEY,
                    `tracking_token`   VARCHAR(64) NOT NULL,
                    `ip_address`       VARCHAR(45) NOT NULL,
                    `country`          VARCHAR(100) DEFAULT 'Unknown',
                    `country_code`     VARCHAR(5) DEFAULT 'XX',
                    ...
                    `device_type`      ENUM('Desktop','Mobile','Tablet','Bot','Unknown') DEFAULT 'Desktop',
                    `operating_system` VARCHAR(64) DEFAULT 'Unknown',
                    `browser`          VARCHAR(64) DEFAULT 'Unknown',
                    `user_agent`       TEXT DEFAULT NULL,
                    `referer`          VARCHAR(500) DEFAULT NULL,
                    `accept_language`  VARCHAR(100) DEFAULT NULL,
                    `privacy_proxy`    TINYINT(1) NOT NULL DEFAULT 0,
                    `proxy_open`       TINYINT(1) NOT NULL DEFAULT 0,
                    `confidence`       ENUM('high','medium','low') DEFAULT 'high',
                    `is_bot`           TINYINT(1) NOT NULL DEFAULT 0,
                    `opened_at`        DATETIME NOT NULL,
                    INDEX `idx_eoe_token` (`tracking_token`),
                    INDEX `idx_eoe_ip` (`ip_address`),
