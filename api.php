...
if ($res === 'track') {
    ...
    $token = trim($_GET['token'] ?? ($_GET['t'] ?? ($action ?? '')));
    ...
    if ($subAction === 'open') {
        ...
        if ($token) {
            try {
                ...
                // Also update master email_tracking & email_open_events
                if (!function_exists('recordTrackingOpenEvent')) {
        ...
        try {
            if ($IS_ADMIN) {
                ...
                $row = db()->query("SELECT
                    ...
                    (SELECT COUNT(DISTINCT from_email) FROM inbound_emails) total_emails,
                    (SELECT COUNT(DISTINCT from_email) FROM inbound_emails WHERE DATE(received_at) = CURDATE()) today_leads,
                    (SELECT COUNT(DISTINCT from_email) FROM inbound_emails WHERE YEAR(received_at) = YEAR(CURDATE()) AND MONTH(received_at) = MONTH(CURDATE())) month_leads,
                ...
                $s=db()->prepare("SELECT
                    ...
                    (SELECT COUNT(DISTINCT i.from_email) FROM inbound_emails i WHERE i.imap_account_id IN ({$imapIdList})) total_emails,
                    (SELECT COUNT(DISTINCT i.from_email) FROM inbound_emails i WHERE i.imap_account_id IN ({$imapIdList}) AND DATE(i.received_at) = CURDATE()) today_leads,
                    (SELECT COUNT(DISTINCT i.from_email) FROM inbound_emails i WHERE i.imap_account_id IN ({$imapIdList}) AND YEAR(i.received_at) = YEAR(CURDATE()) AND MONTH(i.received_at) = MONTH(CURDATE())) month_leads,
...
if ($res === 'email-tracking') {
    ...
    if ($method === 'GET' && $id === 'stats') {
        ...
        if ($totalSent < $totalOpened) {
            $totalSent = max($totalSent, $totalOpened);
        }

        $delivered = max(0, $totalSent);
        $openRate  = $totalSent > 0 ? round(($totalOpened / $totalSent) * 100, 1) : ($totalOpened > 0 ? 100.0 : 0.0);
        ...
        jsonOut([
            ...
            'stats' => [
                ...
                'apple_privacy_opens' => $appleOpens,
                'gmail_proxy_opens'   => $gmailOpens,
            ]
        ]);
    }

    // 2. CAMPAIGNS PERFORMANCE TABLE
    if ($method === 'GET' && $id === 'campaigns') {
        $cWhere = $IS_ADMIN ? "1=1" : "c.user_id = {$UID}";
        try {
            $rows = db()->query(
                "SELECT c.id, c.name,
                        COALESCE(
                            NULLIF((SELECT COUNT(*) FROM email_tracking WHERE campaign_id = c.id), 0),
                            (SELECT COUNT(*) FROM send_logs WHERE campaign_id = c.id AND status='sent'),
                            0
                        ) as sent_count,
                        COALESCE(
                            (SELECT COUNT(*) FROM email_tracking WHERE campaign_id = c.id AND is_opened = 1),
                            (SELECT COUNT(DISTINCT tracking_token) FROM email_open_events WHERE tracking_token IN (SELECT tracking_token FROM email_tracking WHERE campaign_id = c.id)),
                            0
                        ) as opened_count,
                        COALESCE(
                            (SELECT SUM(unique_open_count) FROM email_tracking WHERE campaign_id = c.id),
                            (SELECT COUNT(DISTINCT tracking_token) FROM email_open_events WHERE tracking_token IN (SELECT tracking_token FROM email_tracking WHERE campaign_id = c.id)),
                            0
                        ) as unique_opens,
                        COALESCE(
                            (SELECT SUM(open_count) FROM email_tracking WHERE campaign_id = c.id),
                            (SELECT COUNT(*) FROM email_open_events WHERE tracking_token IN (SELECT tracking_token FROM email_tracking WHERE campaign_id = c.id)),
                            0
                        ) as total_opens,
                        (SELECT MAX(last_open_at) FROM email_tracking WHERE campaign_id = c.id) as last_open_at
                 FROM campaigns c
                 WHERE {$cWhere}
            ...
            foreach ($rows as &$r) {
                ...
                $r['open_rate'] = $s > 0 ? round(($o / $s) * 100, 1) : ($o > 0 ? 100.0 : 0.0);
            }
            jsonOut(['ok' => true, 'rows' => $rows]);
        } catch (\Throwable $e) {
            error_log("[OpenTracking API] Campaigns query error: " . $e->getMessage());
            jsonOut(['ok' => true, 'rows' => []]);
        }
    }
    ...
    if ($method === 'GET' && $id === 'countries') {
        try {
            ...
            error_log("[OpenTracking API] Countries query error: " . $e->getMessage());
            jsonOut(['ok' => true, 'rows' => []]);
        }
    }
    ...
    if ($method === 'GET' && $id === 'devices') {
        try {
            ...
            jsonOut(['ok' => true, 'devices' => [], 'os' => [], 'browsers' => []]);
        }
    }

    // 5. TIMELINE (Hourly & Daily opens)
    if ($method === 'GET' && $id === 'timeline') {
        try {
            ...
            error_log("[OpenTracking API] Timeline query error: " . $e->getMessage());
            jsonOut(['ok' => true, 'hourly' => [], 'daily' => []]);
        }
    }

    // 6. WORLD MAP DATA (Pins & Lat/Lng)
    if ($method === 'GET' && $id === 'map') {
        try {
            ...
            jsonOut(['ok' => true, 'pins' => $pins]);
        } catch (\Throwable $e) {
            error_log("[OpenTracking API] Map query error: " . $e->getMessage());
            jsonOut(['ok' => true, 'pins' => []]);
        }
    }
    ...
    if (($method === 'POST' || $method === 'GET') && $id === 'export') {
        header('Content-Type: text/csv; charset=UTF-8');
        header('Content-Disposition: attachment; filename="email_tracking_export_' . date('Y-m-d_His') . '.csv"');
        $out = fopen('php://output', 'w');
        // UTF-8 BOM for Excel
        fprintf($out, chr(0xEF).chr(0xBB).chr(0xBF));
        ...
        while ($r = $cStmt->fetch()) {
            fputcsv($out, [
                $r['recipient_email'],
                $r['campaign_name'] ?: 'Direct / Auto-Reply',
                $r['sequence_step'] ? '#' . $r['sequence_step'] : 'Initial',
                $r['is_opened'] ? 'Opened' : 'Unopened',
                $r['open_count'],
                $r['unique_open_count'],
                $r['first_open_at'] ?: '—',
                $r['last_open_at'] ?: '—',
    ...
    if ($method === 'GET' && $id && ctype_digit((string)$id)) {
        ...
        $evStmt->execute([$rec['tracking_token']]);
        $events = $evStmt->fetchAll();

        jsonOut(['ok' => true, 'tracking' => $rec, 'events' => $events]);
    }

    // 9. RECIPIENTS LOG TABLE
    if ($method === 'GET') {
        $page = max(1, (int)($_GET['page'] ?? 1));
        $limit = 50;
        $offset = ($page - 1) * $limit;
        ...
        $q = trim($_GET['q'] ?? '');
        if ($q) {
            $where .= " AND (t.recipient_email LIKE ? OR c.name LIKE ?)";
            $params[] = "%{$q}%";
            $params[] = "%{$q}%";
        }
        ...
        if ($status === 'opened') {
            $where .= " AND t.is_opened = 1";
        } elseif ($status === 'unopened') {
            $where .= " AND t.is_opened = 0";
        }

        $totalStmt = db()->prepare("SELECT COUNT(*) FROM email_tracking t LEFT JOIN campaigns c ON c.id = t.campaign_id WHERE {$where}");
        $totalStmt->execute($params);
        $totalCount = (int)$totalStmt->fetchColumn();

        $sql = "SELECT t.*, c.name as campaign_name,
                       (SELECT country FROM email_open_events WHERE tracking_token = t.tracking_token ORDER BY id DESC LIMIT 1) as latest_country,
                       (SELECT country_code FROM email_open_events WHERE tracking_token = t.tracking_token ORDER BY id DESC LIMIT 1) as latest_country_code,
                       (SELECT city FROM email_open_events WHERE tracking_token = t.tracking_token ORDER BY id DESC LIMIT 1) as latest_city,
                       (SELECT device_type FROM email_open_events WHERE tracking_token = t.tracking_token ORDER BY id DESC LIMIT 1) as latest_device,
                       (SELECT browser FROM email_open_events WHERE tracking_token = t.tracking_token ORDER BY id DESC LIMIT 1) as latest_browser,
                       (SELECT privacy_proxy FROM email_open_events WHERE tracking_token = t.tracking_token ORDER BY id DESC LIMIT 1) as latest_privacy_proxy,
                       (SELECT proxy_open FROM email_open_events WHERE tracking_token = t.tracking_token ORDER BY id DESC LIMIT 1) as latest_gmail_proxy
